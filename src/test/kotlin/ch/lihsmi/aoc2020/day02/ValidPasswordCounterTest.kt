...
class ValidPasswordCounterTest {
    ...
    @Test
    fun parsePasswordSampleInputReturnsExpectedCount() {
        val pwdParser = PasswordParser()
        val pwdRules = pwdParser.parse(sampleInput().lines())
        Assertions.assertEquals(1, pwdRules[0].min)
        Assertions.assertEquals(3, pwdRules[0].max)
        Assertions.assertEquals('a', pwdRules[0].char)
        Assertions.assertEquals("abcde", pwdRules[0].password)

        val validPwdCount = pwdParser.countValid(pwdRules)
    ...
    fun parsePasswordInputReturnsExpectedCount() {
        val pwdParser = PasswordParser()
        val pwdRules = pwdParser.parse(quizInput().lines())
        val validPwdCount = pwdParser.countValid(pwdRules)
        Assertions.assertEquals(416, validPwdCount)
    }

    @Test
    fun checkPositionRuleReturnsExpectedCountForSampleInput() {
        val pwdParser = PasswordParser()
        val pwdRules = pwdParser.parse(sampleInput().lines())
        val validPwdCount = pwdParser.countValidByPos(pwdRules)
        Assertions.assertEquals(1, validPwdCount)
    }

    @Test
    fun checkPositionRuleReturnsExpectedCountForQuizInput() {
        val pwdParser = PasswordParser()
        val pwdRules = pwdParser.parse(quizInput().lines())
        val validPwdCount = pwdParser.countValidByPos(pwdRules)
        Assertions.assertEquals(688, validPwdCount)
    }
